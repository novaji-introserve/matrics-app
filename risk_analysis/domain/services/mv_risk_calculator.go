...
func (c *MVRiskCalculator) InitializeCache(ctx context.Context) error {
	...
	// Initialize lastMaxCustomerID for new customer monitoring
	var maxID int64
	if err := c.db.QueryRow(ctx, "SELECT COALESCE(MAX(id), 0) FROM res_partner").Scan(&maxID); err != nil {
		c.logger.Warn("Failed to get initial max customer ID, starting from 0", zap.Error(err))
		maxID = 0
	}
	c.lastMaxCustomerID = maxID
	c.logger.Info("Initialized new customer monitoring baseline",
		zap.Int64("current_max_customer_id", maxID))
...
func (c *MVRiskCalculator) StartNewCustomerMonitor(ctx context.Context, pollInterval time.Duration) {
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				// Query database for max customer ID
				var maxID int64
				err := c.db.QueryRow(ctx, "SELECT COALESCE(MAX(id), 0) FROM res_partner").Scan(&maxID)
				if err != nil {
					c.logger.Error("Failed to query max customer ID", zap.Error(err))
					continue
				}

				// Check if there are new customers
				if maxID > c.lastMaxCustomerID {
					newCustomerIDs := make([]int, 0)

					// Get all new customer IDs
					rows, err := c.db.Query(ctx,
						"SELECT id FROM res_partner WHERE id > $1 ORDER BY id",
						c.lastMaxCustomerID)
					if err != nil {
						c.logger.Error("Failed to query new customers", zap.Error(err))
						continue
					}
					...
					if len(newCustomerIDs) > 0 {
						c.logger.Info("New customers detected",
							zap.Int("count", len(newCustomerIDs)),
							zap.Ints("customer_ids", newCustomerIDs),
							zap.Int64("previous_max_id", c.lastMaxCustomerID),
							zap.Int64("current_max_id", maxID))
						...
						for _, custID := range newCustomerIDs {
							...
							case c.newCustomerChan <- custID:
								c.logger.Debug("Queued new customer for priority processing",
									zap.Int("customer_id", custID))
							case <-ctx.Done():
								c.logger.Info("Context cancelled while queuing new customers")
								return
							}
						}
...
// 	// Get composite computation method from cached settings
// 	c.cacheMu.RLock()
// 	compositeMethod := c.cachedSettings.CompositeComputation
// 	c.cacheMu.RUnlock()

// 	// Calculate composite_risk_score for each customer using configured aggregation method
// 	customerIDs := make([]int, len(validResults))
// 	scores := make([]float64, len(validResults))
// 	levels := make([]string, len(validResults))
// 	compositeScores := make([]interface{}, len(validResults))
...
// 		// Calculate composite score using configured aggregation method (sum, avg, max)
// 		if len(result.CompositePlanLines) > 0 {
// 			planScores := make([]float64, len(result.CompositePlanLines))
// 			for j, line := range result.CompositePlanLines {
// 				planScores[j] = line.RiskScore
// 			}
// 			compositeScores[i] = c.aggregateScoresSlice(planScores, compositeMethod)
// 		} else {
// 			compositeScores[i] = nil
// 		}
// 	}
