...
class RegulatoryDocument(models.Model):
    ...
    source_id = fields.Many2one(
        'regulatory.sources', string='Source', required=True, index=True)
    ...
    def fetch_new_ai_titles(self):
        ...
        titles = self.env['regulatory.document'].search([
        ...
        for title in titles:
            ...
            if not base_url.endswith('/'):
                base_url += '/'
            form_url = f"{base_url}web#id={title.id}&model=regulatory.document&view_type=form"
    ...
    def create(self, vals):
        ...
        record = super(RegulatoryDocument, self).create(vals)
                    ...
                    try:
                        ...
                        for link in pdf_links:
                            ...
                            if pdf_url.endswith(".pdf"):
                                ...
                                existing = self.env["regulatory.document"].sudo().search([
                                    "|",
                                    ("file_name", "=", filename),
                                    ("name", "=", filename_title)
                                ], limit=1)
                                ...
                                # Fetch the source_id
                                source = self.env["regulatory.sources"].sudo().search(
                                    [("name", "ilike", "SEC")], limit=1)
                                if not source:
                                    _logger.error(
                                        "Source 'SEC' not found in the regulatory.sources.")
                                    continue

                                try:
                                    new_record = self.env["regulatory.document"].sudo().create({
                                        "name": filename_title,
                                    ...
                                    document_url = f"{app_base_url}/web#id={new_record.id}&model=regulatory.document&view_type=form"
    ...
    def CBNScraper(self):
        ...
        for doc in documents:
            ...
            existing_title = self.env["regulatory.document"].search(
                [("file_name", "=", doc['refNo'])], limit=1
            )
            existing_name = self.env["regulatory.document"].search(
            ...
            try:
                ...
                # Fetch the source_id
                source = self.env["regulatory.sources"].search(
                    [("name", "ilike", "CBN")], limit=1
                )
                if not source:
                    raise ValueError(
                        "Source 'CBN' not found in the regulatory.sources."
                    )

                # Create the record
                new_record = self.env["regulatory.document"].create({
                    "name": doc['title'],
                    "file": file_binary_data,
                    "file_name": doc['refNo'],
    ...
    def NFIUScraper(self):
        ...
        if not source:
            _logger.error("Source 'NFIU' not found in the regulatory.sources.")
    ...
    def clean_duplicate_records(self):
        ...
        excluded_sources = [
            11] + self.env['regulatory.sources'].search([('name', '=ilike', 'Bank of Industry')]).ids
    ...
    def _send_consolidated_alert_email(self, alert_log, ai_analysis):
        ...
        try:
            ...
            document_url = f"{base_url}/web#id={self.id}&model=regulatory.document&view_type=form"
    ...
    #     """Copy email messages from regulatory.document to keyword.alert.log"""
    #     self.ensure_one()
    ...
    #     """Copy emails to alert logs when posted from regulatory.document"""
    #     message = super(RegulatoryDocument, self).message_post(*args, **kwargs)
