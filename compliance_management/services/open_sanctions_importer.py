...
class OpenSanctionsImporter:
    """
    Importer for OpenSanctions data with SQL-based batch processing for maximum speed
    """
    ...
    def __init__(self, env):
        ...
        """
        self.env = env
        self.cr = env.cr  # Database cursor
        ...
        # Cache field information for faster processing
        self._cache_table_info()
        
    def _cache_table_info(self):
        """Cache table information to avoid repeated queries"""
        # Get column information for res_pep table
        self.cr.execute("""
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name = 'res_pep'
        """)
        self.pep_columns = [row[0] for row in self.cr.fetchall()]
        _logger.info(f"Cached {len(self.pep_columns)} columns for res_pep table")
        
    def process_csv_file(self, file_path, source=None, start_index=0, max_records=None):
        """
        Process CSV file with SQL-based batch processing for maximum speed
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    if has_bom:
                        ...
                        with open(file_path, 'r', encoding='utf-8-sig') as f:
                            ...
                            result = self._process_csv_reader_sql(reader, source, total_records_in_file, start_index, end_index)
                    else:
                        reader = csv.DictReader(f, delimiter=delimiter)
                        result = self._process_csv_reader_sql(reader, source, total_records_in_file, start_index, end_index)
    ...
    def _process_csv_reader_sql(self, reader, source, total_records, start_index, end_index):
        """
        Process a CSV reader object with SQL-based batch processing
        ...
        for current_index in range(start_index, end_index):
            try:
                ...
                if len(batch) >= self.batch_size:
                    try:
                        batch_result = self._process_batch_sql(batch)
                        ...
                        # If transaction failed or cursor closed, get a new cursor
                        if "cursor already closed" in str(e) or "transaction is aborted" in str(e):
                            _logger.warning("Database cursor issue detected, attempting to get a new cursor")
                            self._get_fresh_cursor()
                            # Count as errors
                            total_records_errored += len(batch)
                        else:
                            # Other error, count as errors
                            total_records_errored += len(batch)
                    ...
                    try:
                        ...
                        _logger.error(f"Error committing transaction: {str(e)}")
                        self._get_fresh_cursor()
        ...
        if batch:
            try:
                ...
                _logger.error(f"Error processing final batch: {str(e)}")
                _logger.error(traceback.format_exc())
                total_records_errored += len(batch)
    ...
    def _get_fresh_cursor(self):
        """Get a fresh database cursor when the current one fails"""
        try:
            # Close the old cursor if it's not already closed
            try:
                self.env.cr.close()
            except:
                pass
                
            # Get a new cursor from the registry
            registry = self.env.registry
            self.env.cr = registry.cursor()
            self.cr = self.env.cr
            
            # Recache table info with the new cursor
            self._cache_table_info()
            
            _logger.info("Successfully created a new database cursor")
            return True
        except Exception as e:
            _logger.error(f"Failed to create a new cursor: {str(e)}")
            return False
    ...
    def _process_batch_sql(self, batch):
        """
        Process a batch of records using direct SQL for maximum performance
        ...
        # Get current timestamp for create/write date fields
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        # Get user ID
        user_id = self.env.uid
        
        # Extract unique identifiers from batch
        identifiers = [record.get('unique_identifier') for record in batch if record.get('unique_identifier')]
        
        # Find existing records with SQL
        existing_records = {}
        if identifiers:
            placeholders = ','.join(['%s'] * len(identifiers))
            query = f"SELECT id, unique_identifier FROM res_pep WHERE unique_identifier IN ({placeholders})"
            self.cr.execute(query, identifiers)
            for row in self.cr.fetchall():
                existing_records[row[1]] = row[0]  # Map unique_identifier to id
        ...
        for pep_data in batch:
            try:
                ...
                valid_fields = {k: v for k, v in pep_data.items() if k in self.pep_columns}
                
                if identifier in existing_records:
                    # UPDATE existing record with SQL
                    record_id = existing_records[identifier]
                    
                    # Remove system fields that we'll handle separately
                    for field in ['write_date', 'create_date', 'write_uid', 'create_uid']:
                    ...
                    for field, value in valid_fields.items():
                        if field != 'unique_identifier':  # Don't update the identifier
                            if value == 'NULL':
                                set_clause.append(f"{field} = NULL")
                            else:
                                set_clause.append(f"{field} = %s")
                                values.append(value)
                    
                    # Add write date and user separately to avoid duplication
                    set_clause.append("write_date = %s")
                    values.append(now)
                    set_clause.append("write_uid = %s")
                    values.append(user_id)
                    
                    # Add the record ID at the end
                    values.append(record_id)
                    
                    # Execute the UPDATE
                    query = f"UPDATE res_pep SET {', '.join(set_clause)} WHERE id = %s"
                    self.cr.execute(query, values)
                    updated += 1
                else:
                    # INSERT new record with SQL
                    
                    # Add system fields if not present
                    if 'create_date' not in valid_fields:
                        valid_fields['create_date'] = now
                    if 'write_date' not in valid_fields:
                        valid_fields['write_date'] = now
                    if 'create_uid' not in valid_fields:
                    ...
                    # Execute the INSERT
                    query = f"INSERT INTO res_pep ({', '.join(fields)}) VALUES ({', '.join(placeholders)}) RETURNING id"
                    self.cr.execute(query, values)
                    created += 1
            except Exception as e:
                _logger.error(f"Error processing record with SQL: {str(e)}")
                _logger.error(traceback.format_exc())
                errors += 1
    ...
    def process_api_results(self, api_data, source=None):
        """
        Process API results with SQL-based batch processing
        ...
        try:
            ...
            source_name = source.name if source and hasattr(source, 'name') else 'Unknown API'
            ...
            # Get the entities path from source configuration
            entities_path = 'results'
            if source and hasattr(source, 'api_results_path') and source.api_results_path:
            ...
            for entity in entities:
                try:
                    ...
                    if source and hasattr(source, 'api_entity_filter') and source.api_entity_filter:
    ...
    def map_api_entity_to_pep(self, entity, source=None):
            ...
            try:
                ...
                if 'unique_identifier' not in mapping or not mapping['unique_identifier']:
                    ...
                    if 'id' in entity and entity['id']:
                        ...
                        if name_parts:
                            source_prefix = source.name.replace(' ', '_').lower() if source and hasattr(source, 'name') else 'api'
                            mapping['unique_identifier'] = f"{source_prefix}_api_{'_'.join(name_parts)}".replace(' ', '_')
                        else:
                            # Fallback to a random unique ID
                            import uuid
                            mapping['unique_identifier'] = f"{source.name.replace(' ', '_').lower() if source and hasattr(source, 'name') else 'api'}_api_{uuid.uuid4()}"
        ...
        source_name = source.name if source and hasattr(source, 'name') else 'API import'
        ...
        if mapping.get('name') and not (mapping.get('first_name') and mapping.get('surname')):
            ...
            if len(name_parts) >= 2:
                ...
                mapping['first_name'] = name_parts[0]
                mapping['surname'] = ''  # Use empty value as fallback
