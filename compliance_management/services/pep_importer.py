...
class PepImporter:
    """
    Imports PEP data into the database with improved thread safety
    ...
    def __init__(self, env):
        ...
        # Batch size for database operations - smaller batches for better stability
        self.batch_size = 50
        ...
        # Transaction lock to ensure thread safety
    ...
    def _clean_record(self, record):
        ...
        try:
            ...
            # Trim all strings to avoid database errors
            for field_name, value in record.items():
                if isinstance(value, str) and len(value) > 255:
                    record[field_name] = value[:255]
            
            return record
            
        except Exception as e:
            _logger.error(f"Error cleaning record: {str(e)}")
            _logger.error(traceback.format_exc())
            
            # Ensure at least the required fields exist
            if "unique_identifier" not in record:
                record["unique_identifier"] = f"ERROR-{datetime.now().timestamp()}"
            if "first_name" not in record:
                record["first_name"] = "Error"
            if "surname" not in record:
                record["surname"] = "Record"
    ...
    def _batch_import(self, records):
        """
        Import a batch of records with improved transaction handling and thread safety
        ...
        try:
            # Set the savepoint for partial rollback if needed
            ...
            for record in records:
                try:
                    ...
                    # Use a better domain to find existing records, considering case-insensitive matching
                    existing = self.pep_model.search([
                        "|",
                        ("unique_identifier", "=", record["unique_identifier"]),
                        "&",
                        ("first_name", "=ilike", record["first_name"]),
                        ("surname", "=ilike", record["surname"])
                    ], limit=1)
                    ...
                    if existing:
                        ...
                        changes = False
                        for field, value in record.items():
                            if field in existing and getattr(existing, field) != value:
                                _logger.debug(f"Field {field} changed: '{getattr(existing, field)}' -> '{value}'")
                                changes = True
                                break
                        ...
                        if changes:
                            ...
                            try:
                                ...
                                # Use savepoint to roll back just this record
                                savepoint.rollback()
                                error_count += 1
    ...
    def import_records(self, records):
        """
        Import records into the database with improved batch handling
        ...
        Returns:
            tuple: (records_created, records_updated, records_errored, records_skipped)
        """
        # Split into very small batches for reliability
        batch_created = 0
        batch_updated = 0
        batch_error = 0
        batch_skipped = 0

        # Process in extra small batches for very large imports
        actual_batch_size = min(self.batch_size, max(10, len(records) // 100))
        ...
        for i in range(0, len(records), actual_batch_size):
            ...
            # Small delay to avoid overwhelming the database and give other processes a chance
            if i + actual_batch_size < len(records):
                time.sleep(0.1)
    ...
    def process_file(self, file_info, processor):
        """
        Process and import data from a file with better error handling
        ...
        try:
            ...
            original_created = self.created_count
            original_updated = self.updated_count
            original_error = self.error_count
            original_skipped = self.skipped_count
            
            # Process the file
            _logger.info(f"Processing file for import: {file_info['path']}")
            results = processor.process_file(file_info, self.import_records)

            # Calculate difference in counters
            created = self.created_count - original_created
            updated = self.updated_count - original_updated
            error = self.error_count - original_error
            skipped = self.skipped_count - original_skipped
            ...
            return {
                ...
                "records_errored": error,
                "records_skipped": skipped,
                "errors": self.errors[-10:],  # Include last 10 errors for reference
