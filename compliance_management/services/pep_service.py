        ...
        try:
            ...
            if "-of-" in entity_id:
                ...
                if names:
                    ...
                    if len(names) > 1:
                        main_parts = entity_id.split("-of-")[0].split("-")
                        main_last_name = None
                        
                        # Try to extract last name from main entity
                        for i in range(len(main_parts)-1, -1, -1):
                            if main_parts[i] not in ["ng", "dots", "district", "region"] and not main_parts[i].isdigit() and len(main_parts[i]) > 1:
                                main_last_name = main_parts[i]
                                break
                                
                        if main_last_name:
                            for name in names:
                                name_parts = name.lower().split()
                                # If this name shares a part with the main entity's last name
                                # and it's in a format that suggests a relationship
                                if main_last_name in name.lower():
