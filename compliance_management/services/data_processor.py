...
class DataProcessor:
    ...
    with improved memory management and file type handling
    """
    ...
    def __init__(self, env=None):
        ...
        self.batch_size = 250  # Reduced batch size for better memory usage
    ...
    def _adaptive_batch_size(self, df_size):
        """
        Calculate optimal batch size based on dataframe size
        
        Args:
            df_size: Number of rows in the dataframe
            
        Returns:
            int: Appropriate batch size
        """
        if df_size > 10000:
            return 50  # Very small batches for huge files
        elif df_size > 5000:
            return 100
        elif df_size > 1000:
            return 200
    ...
    def process_csv(self, file_path, source_name, callback=None):
        """
        Process a CSV file with memory-efficient handling
        ...
        try:
            ...
            preprocessed_path = self._preprocess_csv_file(file_path)
            if preprocessed_path:
                _logger.info(f"Using preprocessed file: {preprocessed_path}")
                actual_file_path = preprocessed_path
            else:
            ...
            # First try reading with pandas directly with various settings
            parse_methods = [
                # Method 1: Default UTF-8 with comma delimiter
                {'encoding': 'utf-8', 'sep': ','},
                # Method 2: Latin-1 encoding with comma delimiter
                {'encoding': 'latin-1', 'sep': ','},
                # Method 3: UTF-8 with semicolon delimiter
                {'encoding': 'utf-8', 'sep': ';'},
                # Method 4: Latin-1 with semicolon delimiter
                {'encoding': 'latin-1', 'sep': ';'},
                # Method 5: UTF-8 with tab delimiter
                {'encoding': 'utf-8', 'sep': '\t'},
                # Method 6: CP1252 with comma
                {'encoding': 'cp1252', 'sep': ','}
            ]

            for method in parse_methods:
                try:
                    _logger.info(f"Trying pandas parse with {method}")
                    df = pd.read_csv(
                        actual_file_path,
                    ...
                    if df is not None and not df.empty and len(df.columns) > 1:
                        _logger.info(f"Successfully parsed with {method}")
                        break
                    else:
                        df = None
                except Exception as e:
                    _logger.warning(f"Method {method} failed: {str(e)}")
                    df = None

            # If pandas methods failed, try with Python's CSV module
            if df is None:
                try:
                    ...
                    for enc in encodings:
                        try:
                            with open(actual_file_path, 'r', encoding=enc) as f:
                                ...
                                content = f.read(4096)
                                if content:
                                    _logger.info(f"Detected encoding: {enc}")
                                    break
                        except UnicodeDecodeError:
                            continue

                    if not content:
                        _logger.error("Could not detect file encoding")
                        return {
                            "status": "error",
                    ...
                    rows = []
                    with open(actual_file_path, 'r', encoding=enc, errors='replace') as f:
                        ...
                        if not headers:
                            _logger.error("No headers found in CSV file")
                            return {
                                "status": "error",
            ...
            if df is None:
                _logger.error("All parsing methods failed for CSV file")
                return {
                    "status": "error",
                    "message": "Could not parse CSV file with any method",
                    "records_processed": 0,
                    "records_valid": 0,
                    "records_invalid": 0,
                }
            ...
            rows_count = len(df)
            batch_size = self._adaptive_batch_size(rows_count)
            ...
            if preprocessed_path and preprocessed_path != file_path:
                try:
                    ...
                    _logger.info(f"Removed preprocessed file: {preprocessed_path}")
                except:
                    pass
    ...
    def _preprocess_csv_file(self, file_path):
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                content = f.read(8192)  # Read first 8K to check for issues
            ...
            with open(file_path, 'rb') as in_file, open(cleaned_path, 'wb') as out_file:
                ...
                # Skip BOM if present
                content = in_file.read(3)
                if content == b'\xef\xbb\xbf':
                    _logger.info("Removing BOM marker")
                else:
                    # Reset position and reread
                    in_file.seek(0)
                    content = in_file.read(chunk_size)
    ...
    def _safe_process_ods_file(self, file_path, source_name, callback=None):
        ...
        try:
            ...
            if callback:
                placeholder_record = {
                    "unique_identifier": f"PLACEHOLDER-{source_name}-{int(datetime.now().timestamp())}",
                    "first_name": "File",
                    "surname": "Skipped",
    ...
    def _safe_process_excel_large(self, file_path, source_name, file_extension, callback=None):
        ...
        try:
            ...
            return {
                "status": "success",
                "message": f"Processed Excel file: {records_valid} valid records, {records_invalid} invalid (limited to 100 rows)",
                "records_processed": records_valid + records_invalid,
                "records_valid": records_valid,
                "records_invalid": records_invalid,
            }
    ...
    def process_pdf(self, file_path, source_name, callback=None):
        """
        Process a PDF file with improved memory handling
        ...
        try:
            ...
            # Extract tables from PDF with memory limits
            try:
                # Limit to first 10 pages for large PDFs
                tables = tabula.read_pdf(
                    file_path, 
                    pages="1-10", 
                    multiple_tables=True,
                    guess=True,
                    stream=True  # Stream mode for better memory usage
                )
            except Exception as e:
                _logger.warning(f"Error extracting tables with default settings: {str(e)}")
                # Fallback to simpler extraction
                tables = tabula.read_pdf(
                    file_path, 
                    pages="1-5",  # Even fewer pages
                    multiple_tables=True,
                    lattice=True  # Try lattice mode for better table detection
                )
            ...
            for i, df in enumerate(tables):
                ...
                batch_size = 50  # Small batches for PDFs
    ...
    def process_xml(self, file_path, source_name, callback=None):
        """
        Process an XML file with memory-efficient streaming parsing
        ...
        try:
            ...
            # Try to identify the record element tag
            potential_tags = [
                "sdnEntry",
                "record",
                "entity",
            ...
            # First determine the record element tag by scanning the first part of the file
            record_tag = None
            tag_counts = {tag: 0 for tag in potential_tags}

            # Open file and check first few KB for possible tags
            with open(file_path, "rb") as f:
                sample = f.read(10240)  # Read 10KB sample

                for tag in potential_tags:
                    count = sample.lower().count(f"<{tag}".encode())
                    tag_counts[tag] = count

            # Select tag with highest count
            max_count = 0
            ...
            if not record_tag:
                # Fallback to manually parsing the whole file
                _logger.warning(
                    f"Could not determine record element tag, using manual parsing"
                )
                tree = ET.parse(file_path)
                root = tree.getroot()

                # Find all person/entity elements using different tags
                person_elements = []

                # Try different tag patterns commonly used in sanctions XML files
                for pattern in potential_tags:
                ...
                for element in person_elements:
                    ...
                    if not column_map:
                        _logger.warning("No field mappings found for XML element")
                ...
                _logger.info(f"Using iterative parsing with record tag: {record_tag}")

                # Track tags at current level
                current_record = None
                current_field = None
                field_text = ""
                ...
                # Set a limit for large files
                max_records = 10000  # Limit to 10,000 records

                for event, elem in ET.iterparse(file_path, events=("start", "end")):
                    # Check if we've hit our record limit
                    if processed_count >= max_records:
                        _logger.warning(f"Reached maximum record limit of {max_records}")
                        break
                    ...
                    # Record start
                    if event == "start" and tag == record_tag:
                        current_record = {}
                        # Add attributes
                        for attr_name, attr_value in elem.attrib.items():
                            if "}" in attr_name:  # Handle namespace
                                attr_name = attr_name.split("}", 1)[1]
                            if attr_value:
                                current_record[attr_name] = attr_value
                    ...
                    # Field start
                        ...
                        current_field = tag
                        field_text = elem.text if elem.text else ""
                    ...
                    # Field end
                        ...
                        and current_record is not None
                        and tag != record_tag
                        and tag == current_field
                    ):
                        # Store field value if not empty
                        if field_text and field_text.strip():
                            if tag in current_record:
                                # If field already exists, convert to list or append
                                if isinstance(current_record[tag], list):
                                    current_record[tag].append(field_text.strip())
                                else:
                                    current_record[tag] = [
                                        current_record[tag],
                                        field_text.strip(),
                                ...
                                current_record[tag] = field_text.strip()
                    ...
                    # Record end
                    elif event == "end" and tag == record_tag:
                        if current_record:
                            processed_count += 1

                            # Map fields using source-specific or generic mapping
                            column_map = self._get_mapping(
                                source_name, current_record.keys()
                            )

                            prepared_record = self._prepare_record(
                                current_record, column_map, source_name
                            )
                            ...
                            if len(batch_records) >= batch_size:
                                ...
                                # Log progress
                                _logger.info(
                                    f"Processed {processed_count} records from XML file"
                                )
            ...
            _logger.error(f"XML parsing error in file {file_path}: {str(e)}")
            return {
                "status": "error",
            ...
            _logger.error(f"Permission denied accessing file: {file_path}")
            return {
                "status": "error",
            ...
            _logger.error(f"Memory error processing XML file: {file_path}")
            return {
                "status": "error",
                "message": "Not enough memory to process this XML file",
                "records_processed": 0,
                "records_valid": 0,
                "records_invalid": 0,
            }
    ...
    def process_file(self, file_info, callback=None):
        """
        Process a file based on its type with comprehensive error handling
        ...
        try:
            ...
            if not os.path.isfile(file_path):
                _logger.error(f"Not a file: {file_path}")
                return {
                    "status": "error",
            ...
            if os.path.getsize(file_path) == 0:
                _logger.error(f"Empty file: {file_path}")
                return {
                    "status": "error",
                    "message": f"Empty file: {file_path}",
                    "records_processed": 0,
                    "records_valid": 0,
                    "records_invalid": 0,
                }
            ...
            if file_type == "csv":
                ...
                # For text files, try processing as CSV first
                _logger.info(f"Treating .txt file as potential CSV: {file_path}")
                result = self.process_csv(file_path, source_name, callback)
                if result["records_processed"] > 0:
                    return result
                else:
                    _logger.warning(f"Could not process text file as CSV. Treating as unsupported format.")
                    return {
                        "status": "warning",
                        "message": "Text file could not be processed as CSV",
                        "records_processed": 0,
                        "records_valid": 0,
                        "records_invalid": 0,
                    }
            ...
            _logger.error(f"Unexpected error processing file: {str(e)}")
            _logger.error(traceback.format_exc())
            return {
                "status": "error",
