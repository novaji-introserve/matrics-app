...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            # logging.StreamHandler()  # Add this line to see logs in console
        ]
    )

setup_logging()
# Call this at the start of your script
logger = logging.getLogger("__TRANSACTIONS__")
config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'configs', 'trans_config.json')
...
class ETLManager:
    ...
    def _create_sync_table(self, target_cursor):
        ...
        target_cursor.execute("""
            CREATE TABLE IF NOT EXISTS sync_timestamps (
                ...
                row_hashes JSONB,
                use_hash_table BOOLEAN NOT NULL DEFAULT FALSE
            );
        """)
        
        # Create separate table for large hash storage
        logger.debug("Ensuring row_hashes table exists...")
        target_cursor.execute("""
            CREATE TABLE IF NOT EXISTS row_hashes (
                table_name VARCHAR(255) NOT NULL,
                row_key VARCHAR(255) NOT NULL,
    ...
    def update_transaction_branch_ids(self):
        """Updates branch_id column in transactions table using branch_code lookup."""
        logger.info("Starting transaction table branch_id update process...")
        start_time = time.time()
        
        try:
            with self.get_target_connection() as target_conn:
                # First get the count for reporting purposes
                with target_conn.cursor() as count_cursor:
                    ...
                    if count == 0:
                        logger.info("No transaction records need branch_id updates.")
                        return
                    
                    logger.info(f"Found {count} transactions that need branch_id updates.")
                
                # Use a server-side cursor for memory-efficient pagination
                with target_conn.cursor(name='update_cursor') as fetch_cursor:
                    fetch_cursor.itersize = 10000  # Number of rows to fetch at once
                    
                    # Get all IDs that need updating, ordered for consistent batching
                    ...
                    # Process in batches using regular cursor for updates
                    with target_conn.cursor() as update_cursor:
                        while True:
                            # Fetch a batch of IDs
                            batch_ids = fetch_cursor.fetchmany(batch_size)
                            
                            if not batch_ids:
                                break  # No more records
                            
                            # Extract IDs into a list
                            id_list = [row[0] for row in batch_ids]
                            
                            # Update only these specific IDs
                            update_query = """
                            UPDATE res_customer_transaction t
                            SET branch_id = (
                                SELECT rb.id 
                                FROM res_branch rb 
                                WHERE rb.co_code = t.branch_code 
                                LIMIT 1
                            )
                            WHERE t.id = ANY(%s)
                            """
                            update_cursor.execute(update_query, (id_list,))
                            
                            batch_updated = update_cursor.rowcount
                            updated_count += batch_updated
                            target_conn.commit()
                            
                            logger.info(f"Updated batch of {batch_updated} transactions, total updated: {updated_count}/{count}")
                    
                    elapsed_time = time.time() - start_time
                    logger.info(f"Transaction branch_id update completed. Updated {updated_count} records in {elapsed_time:.2f} seconds.")
                    
        except Exception as e:
            logger.error(f"Error updating transaction branch_ids: {e}")
            raise
    ...
    #                 # Update using a WITH clause to handle lookup in batches
    #                 update_query = """
    #                 WITH batch AS (
    #                     SELECT t.id
    ...
    #                 while True:
    #                     target_cursor.execute(update_query, (batch_size,))
    #                     batch_results = target_cursor.fetchall()
    #                     batch_updated = len(batch_results)
    ...
    def lookup_value(
        ...
        # NEW CODE: Convert key_value to string if the column name contains 'code'
        original_key_value = key_value
        if 'code' in key_column.lower() and not isinstance(key_value, str):
            key_value = str(key_value)
            logger.debug(
                f"Converting lookup key from {type(original_key_value).__name__} '{original_key_value}' to string '{key_value}' for column {key_column}")
        ...
        try:
            ...
            if result:
                ...
                # Store in cache using the ORIGINAL key value type to maintain consistency
                self.lookup_cache.set(
                    table, key_column, original_key_value, value_column, found_value)
                return found_value
            else:
                logger.warning(
                ...
                self.lookup_cache.set(
                    table, key_column, original_key_value, value_column, None)
                return None
        except psycopg2.Error as e:
            logger.error(
    ...
    def run_etl_cycle(self):
        ...
        try:
            ...
            # self.update_transaction_branch_ids()
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'tables' not in etl_config:
            raise ValueError(
                "trans_config.json must contain a 'tables' section.")
        ...
        logger.error(
            f"Configuration file not found: {e}. Please ensure db_config.json and trans_config.json exist.")
