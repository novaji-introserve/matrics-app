...
class DynamicChartController(http.Controller):
    """
    Enhanced Dynamic Chart Controller with server-side caching
    
    Provides API endpoints for dashboard charts with filestore caching for improved performance.
    """
    ...
    def extract_where_clauses(self, sql_query):
        """
        Extract all clauses attached to the WHERE statement in an SQL query
        
        Args:
            sql_query (str): SQL query
            
        Returns:
            list: List of WHERE clauses
        """
        # Normalize the query
        sql_query = ' '.join(sql_query.strip().split())
    ...
    def _extract_table_aliases(self, sql_query):
        ...
        Returns:
            dict: Dictionary mapping aliases to table names
        """
        aliases = {}
    ...
    def _map_field_name(self, field, table_aliases, field_patterns):
        ...
        Args:
            ...
            table_aliases (dict): Table aliases
            field_patterns (dict): Field patterns
            
        Returns:
            str: Mapped field name
        """
        if '.' not in field:
            return field  # No table prefix, return as is
    ...
    def _add_where_to_query(self, query, where_clause):
        ...
        Args:
            ...
            where_clause (str): WHERE clause to add
    ...
    def _process_query_results(self, chart, query):
        """
        Process results from SQL query for chart rendering
        
        Args:
            chart (Model): Chart record
            query (str): SQL query to execute
            
        Returns:
            dict: Processed chart data
        """
        try:
            request.env.cr.execute(query)
            results = request.env.cr.dictfetchall()
        except Exception as e:
            _logger.error(f"Error executing query for chart {chart.name}: {e}")
            return None
        ...
        # Extract the IDs if we found a suitable field
        ids = [r[id_field] if id_field and id_field in r else None for r in results]
        ...
        values = [float(r.get(y_field, 0) or 0) for r in results]
        
        # Generate colors
        colors = self._generate_colors(chart.color_scheme, len(results))
        
        # Build non-date domain filter
        domain_filter = self.sql_where_to_odoo_domain_no_dates(query)
    ...
    def get_chart_data(self, cco, branches_id, datepicked, skip_cache=False, **kw):
        """
        Get chart data with caching
        
        Args:
            cco (bool): CCO status flag
            branches_id (list): List of branch IDs
            datepicked (int): Date range in days
            skip_cache (bool): Skip cache for direct data fetching
        ...
        is_cco = cco if cco is not None else user.has_group('compliance_management.group_compliance_chief_compliance_officer')
        is_bco = user.has_group('compliance_management.group_compliance_branch_compliance_officer')
        is_co = user.has_group('compliance_management.group_compliance_compliance_officer')
        is_crm = user.has_group('compliance_management.group_compliance_compliance_risk_manager')
        
        # Get group key for cache identification
        group_key = 'cco' if is_cco else 'bco' if is_bco else 'co' if is_co else 'crm' if is_crm else 'regular'
        
        # Generate cache key based on parameters
        cache_key = f"dashboard_charts_{datepicked}"
        
        # Check cache first (unless skip_cache is True)
        if not skip_cache:
            cache_model = request.env['compliance.dashboard.cache']
            cached_data = cache_model.get_cache(cache_key, group_key)
            
            if cached_data:
                return cached_data
        
        # Cache miss or skip_cache, fetch fresh data
        ...
        odoo_current_date = f"{today} {time_23_59_59}"
        odoo_prev_date = f"{prev_date} {time_00_00_00}"
        ...
        for chart in charts:
            ...
            try:
                ...
                where_clause = f"{chart.date_field} >= '{odoo_prev_date}'" if datepicked == 20000 else f"{chart.date_field} BETWEEN '{odoo_prev_date}' AND '{odoo_current_date}'"
                
                # Add branch filtering if needed
                if not is_cco and chart.branch_filter and branches_id:
                    if isinstance(branches_id, list):
                        ...
                        where_clause += f" AND {chart.branch_field} = {branches_id}"
                elif not is_cco and chart.branch_filter:
                ...
                if not result or (result.get('labels') == [] and result.get('type') == ''):
                    continue
                
                charts_data.append(result)
                
            except Exception as e:
                _logger.error(f"Error processing chart {chart.name}: {e}")
                continue
        
        # Cache the result
        if not skip_cache:
            cache_model = request.env['compliance.dashboard.cache']
            cache_model.set_cache(cache_key, charts_data, group_key)
