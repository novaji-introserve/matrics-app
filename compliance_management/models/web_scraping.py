...
#     @api.model 
#     def manual_fetch_for_source(self, source_id, job_type='both'):
#         """
#         Manually trigger a fetch job for a specific source
        
#         Args:
#             source_id: ID of the source to fetch
#             job_type: Type of job to run ('csv', 'api', 'both')
...
#             job = self._create_api_job(source)
#         else:
#             # Create a 'both' type job
#             job = self.env['opensanctions.job.queue'].create({
#                 'name': f"Full Import - {source.name}",
#                 'source_id': source.id,
#                 'job_type': 'both',
#                 'priority': 20 if source.is_opensanctions else 10,
#                 'state': 'pending'
#             })
