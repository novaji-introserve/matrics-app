...
def process_file(file_path):
    """Process a single file, replacing 'compliance_management' with 'altbank_addons'
    ...
    try:
        ...
        # Apply replacements
        new_content = pattern1.sub(lambda m: m.group(0).replace('compliance_management.', 'altbank_addons.'), content)
        new_content = pattern2.sub(lambda m: m.group(0).replace('compliance_management.', 'altbank_addons.'), new_content)
