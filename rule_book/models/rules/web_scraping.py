...
class ExternalResource(models.Model):
    ...
    def NDICScrapper(self):
        ...
        pdf_links = soup.select(
            "ul li a[href]"
        )  # Assuming the first tag selects the PDF link
        file_names = soup.select(
            "li a"
        )  # Assuming the second tag selects the file names
            ...
            if pdf_url.endswith(".pdf"):
                ...
                try:
                    ...
                    record_data = {
                        ...
                        # "released_date": released_date,  # Release date from the filename
                        "status": "active",  # Default status to 'active'
                    ...
                    print(record_data)
                    # self.env["rulebook.title"].create(record_data)

                except Exception as e:
                    raise ValueError(f"Error processing file '{filename}': {str(e)}")

        return "Scrape for NDIC was successful!"
            ...
            for url, html_tag in zip(urls, html_tags):
                ...
                for link, file_name in zip(pdf_links, file_names):
                    ...
                    if pdf_url:
                        ...
                        filename = (
                            f"{unquote(noSpaceFileName)[:-len(date_string)-1]}{date_string}.pdf"
                        )
                        # return filename
                        pdf_response = requests.get(pdf_url)
                        ...
                        try:
                            ...
                            file_binary_data = self.download_file_as_binary(pdf_url)
                            date_string = self.get_date_from_url(unquote(pdf_url))
                            released_date =  self.parse_date(date_string)  if  date_string else None
