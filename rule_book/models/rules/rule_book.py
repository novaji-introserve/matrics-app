...
class Rulebook(models.Model):
    ...
    date_value = fields.Datetime(
        string='Date',
        help="Specify the exact date for the regulatory action."
    )

    day_value = fields.Integer(
        string='Day',
        help="Specify the day of the month for the regulatory action."
    )
    ...
    def _compute_date(self):
        ...
        default_time = datetime.combine(today, datetime.min.time()).replace(hour=8, minute=0, second=0)
        for record in self:
            if record.frequency_type == 'date':
                ...
                record.computed_date = default_time.replace(year=year, month=next_quarter_month)
                record.is_recurring = True
                ...
                if record.month_value:
                    record.computed_date = default_time.replace(month=int(record.month_value))
                    record.is_recurring = True
    ...
    def _compute_internal_due_date(self):
        for record in self:
            if record.escalation_date and record.internal_due_date_unit in [
                ...
                delta_args = {record.internal_due_date_unit: -record.internal_due_date_value}
    ...
    def _compute_escalation_date(self):
        for record in self:
            if record.computed_date and record.escalation_date_unit in ['days', 'hours', 'minutes', 'seconds']:
                delta_args = {record.escalation_date_unit: -record.escalation_date_value}
    ...
    def _schedule_due_dates(self):
        for record in self:
            ...
            events = self.env['calendar.event'].search([('name', 'ilike', f'Regulatory Due Date for {record.name}')])
            events.unlink()
            ...
            if record.computed_date:
                self.env['calendar.event'].create({
                    ...
                    'stop': record.computed_date + timedelta(hours=1),  # Set duration of 1 hour
            ...
            if record.internal_due_date:
                self.env['calendar.event'].create({
                    ...
                    'stop': record.internal_due_date + timedelta(hours=1),  # Set duration of 1 hour
                    'allday': False,  # Event is not all day; includes specific time
                })
            ...
            if record.escalation_date:
                self.env['calendar.event'].create({
                    ...
                    'stop': record.escalation_date + timedelta(hours=1),  # Set duration of 1 hour
                    'allday': False,  # Event is not all day; includes specific time
                })
    ...
    def _compute_next_due_date(self):
        ...
        for record in self:
            ...
            if record.frequency_type == "monthly":
                next_due_date = record.computed_date + relativedelta(
                            months=1
                        )
            elif record.frequency_type == "quarterly":
                next_due_date = record.computed_date + relativedelta(
    ...
    def send_reminder_email(self):
        ...
        for rulebook in self:
            if rulebook.report_status != 'completed' and rulebook.internal_due_date <= fields.Datetime.now():
            ...
            if rulebook.escalation_date <= fields.Datetime.now() and rulebook.report_status != 'completed':
        ...
        for record in self:
            if record.is_recurring:
                ...
                if record.frequency_type == 'monthly':
                    ...
                    next_due_date = record.computed_date.replace(year=record.computed_date.year + 1)
                elif record.frequency_type == 'immediate':
                    next_due_date = record.computed_date
                else:
                    next_due_date = record.computed_date  # Default to current date if none specified
